...
def create_patients_table(cur, conn):
    ...
    cur.execute("CREATE TABLE Patients (pet id INTEGER, name TEXT, species_id INTEGER, age INTEGER, cuteness INTEGER, aggressivenes INTEGER)")
    conn.commit()
...
def add_fluffle(cur, conn):
    cur.execute("INSERT INTO Patients (pet id, name, species_id, age, cuteness, agressiveness) VALUES (?,?,?,?,?,?)", (0, "Fluffle", 0, 3, 90, 100))
    conn.commit()
...
def add_pets_from_json(filename, cur, conn):
    ...
    for item in json_data:
        ...
        agressiveness = int(item['agressiveness'])
        cur.executive('SELECT id from Species WHERE title = ?', (species, )) #(species, ) is a tuple
        species_id = int(cur.fetchone()[0])
        # print(patient_id, name, species_id, age, cuteness, agressiveness)
        cur.execute("INSERT INTO Patients (pet id, name, species_id, age, cuteness, agressiveness) VALUES (?,?,?,?,?,?)", (patient_id, name, species_id, age, cuteness, agressiveness))
        patient_id += 1 
